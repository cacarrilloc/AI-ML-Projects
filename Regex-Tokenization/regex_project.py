...
Original file is located at
    https://colab.research.google.com/drive/1IUHWNHaZX-WVaSAsWUSX6jlKDSOTBqs8

# Intro to Regular Expressions and Tokenization
...
# can you understand what it is trying to do?
date_pattern = '[A-Za-z]{3,4} \d{1,2}, \d{4}'
...
def find_email(text):
    ...
    email_lst = r'[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(?:edu|com)'
    return re.findall(email_lst, text)


emails_found = find_email(penn_online_text)
# this should return nothing if CCB's email is found
assert 'ccb@upenn.edu' in emails_found, "CCB's email not found"
...
### Read text data ###
### DO NOT CHANGE  ###
q13_text = get_text_from_url(
    'https://simple.wikipedia.org/wiki/List_of_people_from_New_Jersey')
...
def whitespaceTokenizerUpdate(text):
    ...
    # Normalize multiple spaces and split by whitespace
    tokens = text.split()
...
assert whitespaceTokenizerUpdate(
    text) == ["This", "212", ",", "is", "a", "test", "!"]
...
def nltk_tokenize_content(text):
    ...
    # Filter: keep only alphabetic tokens and exclude stopwords
...
assert nltk_tokenize_content(text) == [
    'good', 'muffins', 'cost', 'new', 'york', 'please', 'buy', 'two', 'thanks']
...
def stanza_tokenizer(text):
    ...
    document = nlp(text)
    tokens = [
        token.text for sentence in document.sentences for token in sentence.tokens]
    return tokens


text = "Good muffins cost $3.88\nin New (York).  Please (buy) me\ntwo of them.\n(Thanks)."
assert stanza_tokenizer(text) == ['Good', 'muffins', 'cost', '$', '3.88', 'in', 'New', '(', 'York', ')',
...
def compute_word_freq(corpus, threshold=50):
    ...
    valid_freqs = {
        word: count for word, count in word_freqs.items() if count >= threshold}
...
# this line might take up to 10 minutes to finish, might be worth it to test it on a small corpus first (e.g. a few sentences)
word_freqs = compute_word_freq(corpus)
### DO NOT CHANGE ###
...
vocab_size = 5000  # or smaller for testing
vocab = set(base_vocab)  # make sure it's a set for fast lookup
merges_records = {}  # save learned merge rules
splits_copy = copy.deepcopy(splits)  # avoid modifying original

while len(vocab) < vocab_size:
    # Compute pair frequencies
    pair_freqs = compute_pair_freqs(word_freqs, splits_copy)

    # Stop if no pairs are found
    if not pair_freqs:
        break
    ...
    if len(vocab) % 500 == 0:
        print(
            f"Vocab size: {len(vocab)} - Latest merge: ({a}, {b}) -> {merged_token}")

end = time.time()
print(f"BPE training completed in {round(end - start, 2)} seconds")
...
def BPE_tokenize(text, merges_records):
    ...
    words = nltk_tokenize_content(text)
    ...
    for (a, b), ab in merges_records.items():
        for idx, token_list in enumerate(splits):
            new_tokens = []
            i = 0
            while i < len(token_list):
                if i < len(token_list) - 1 and token_list[i] == a and token_list[i + 1] == b:
            ...
            splits[idx] = new_tokens  # Update tokenized word

    return sum(splits, [])  # flat list of tokens
